...
class Anak(AhliWaris):
    ...
    def total_unit_ashabah(self): 
        # Laki-laki = 2 unit, Perempuan = 1 unit
        return self._laki * 2 + self._perempuan * 1
    def get_bagian(self, harta, ctx): 
        # Bagian anak selalu dihitung sebagai residu (ashabah) atau fixed share
        # di dalam kelas WarisanCalculator
        return 0 

class WarisanCalculator:
    """Logika inti perhitungan warisan Faraidh."""
    def __init__(self):
        # Inisialisasi objek ahli waris
        self.ayah = Ayah()
        self.ibu = Ibu()
        self.suami = Suami()
        self.istri = Istri()
    ...
    def compute(self, total, ayah=False, ibu=False, suami=False, istri=False, anak_laki=0, anak_perempuan=0):
        ...
        # 1. Hitung bagian tetap (Ashabul Furudh)
        # Bagian Ayah (fixed 1/6) hanya dihitung jika ada anak, sisanya (ashabah) ditangani nanti
        ...
        # Bagian Anak Perempuan saja (fixed share)
        ...
        # 2. Aturan Awl (jika total fixed > total dan tidak ada anak laki-laki)
        if total_fixed > total and anak_laki == 0:
            scale = total / total_fixed
            # Scale down semua fixed shares secara proporsional
        ...
        # Jika ada anak laki-laki (Ashabah Bil-Ghair) ATAU Ayah (Ashabah Bin-Nafs)
        
        # Ashabah anak-anak (sisa dibagi 2:1)
        ...
        # Ashabah Ayah (jika tidak ada anak sama sekali)
        elif total_anak == 0 and ayah and sisa > 0:
            # Ayah mengambil sisa (menjadi Ashabah Bin-Nafs setelah mengambil 1/6)
        ...
        if sisa > 0:
            # Jika masih ada sisa, tampilkan sebagai yang tidak terdistribusi
...
class AppWarisanUI:
    ...
    def show_riwayat_view(self):
        ...
        # Action buttons for Riwayat
        actions = tk.Frame(frame, bg=PANEL_BG)
        actions.pack(side="bottom", fill="x", padx=12, pady=(0,8))
        tk.Button(actions, text="Reset Riwayat", bg=BTN_RESET, fg=WHITE, command=self.action_clear_history, bd=0).pack(side="left", padx=6)
    ...
    def action_reset_inputs(self):
        ...
        try:
            ...
            # Mengatasi error jika belum di view hitung
    ...
    def action_clear_history(self):
        ...
        if messagebox.askyesno("Konfirmasi", "Yakin ingin menghapus semua riwayat perhitungan?"):
            ...
            if hasattr(self, "lb"): self.lb.delete(0, tk.END)
            if hasattr(self, "txt_riwayat_detail"): self.txt_riwayat_detail.delete("1.0", tk.END)
            messagebox.showinfo("Sukses", "Riwayat telah dihapus.")
    ...
    def hitung_warisan(self):
        try:
            ...
            if harta <= 0: raise ValueError("Total harta harus lebih besar dari 0.")
        ...
        # Jika sedang di view riwayat, otomatis tampilkan detailnya
    ...
    def show_riwayat_detail_from_entry(self, entry):
        ...
        # Mapping nama input agar lebih rapi
        input_labels = {
            'ayah': 'Ayah', 'ibu': 'Ibu', 'suami': 'Suami', 'istri': 'Istri',
